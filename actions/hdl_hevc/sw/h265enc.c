                    ...
                    )
{
    ...
    if ((fp1 = fopen("/afs/bb/u/lyhlu/vol2/HEVC/20181101/snap/actions/hdl_h265enc/sw/bs1.dat","wb"))==NULL) {
            VERBOSE0("ERROR: Fail to create bs1 file!\n");
            return -1;
    }

    if ((fp2 = fopen("/home/chgu/Desktop/oc-accel/actions/hdl_hevc/sw/e_c_top_bs_i_W8.dat","r"))==NULL) {
            VERBOSE0("ERROR: Fail to read bs check file!\n");
            return -1;
    }
    ...
    // init
    action_write(dnc, REG_FLG_IRQ, 1);
    action_write(dnc, REG_X_TOTAL, X_TOTAL);
    action_write(dnc, REG_Y_TOTAL, Y_TOTAL);
    action_write(dnc, REG_FLG_SAO_LU, FLAG_SAO_LU);
    action_write(dnc, REG_FLG_SAO_CH, FLAG_SAO_CH);
    ...
    // source address
    addr = (uint64_t)src;
    action_write(dnc, REG_ORI_LU_BASE_HIGH, (uint32_t)(addr >> 32));
    action_write(dnc, REG_ORI_LU_BASE_LOW, (uint32_t)(addr & 0xffffffff));
    addr = (uint64_t)src + FRAMEWIDTH * FRAMEHEIGHT ;
    action_write(dnc, REG_ORI_CH_BASE_HIGH, (uint32_t)(addr >> 32));
    action_write(dnc, REG_ORI_CH_BASE_LOW, (uint32_t)(addr & 0xffffffff));
    ...
    for(frame_cnt = 0; frame_cnt < 1; frame_cnt++) {
        ...
        action_write(dnc, REG_START, 1);

        // Poll status for done signal

        VERBOSE0 ("Done!\n");
        while ((action_read(dnc, REG_RUN) & 0x00000003) != 0) {
        	;
        }
        ...
        // get bs length
        bs_length = action_read(dnc, REG_BS_LEN);
        ...
        for (uint32_t i = 0; i < bs_length; i++) {
        	...
        	int dut_dat;
        	fscanf(fp2, "%d\n", &ref_dat);
        	dut_dat = *((uint8_t *)dest + i) ;
        	if (ref_dat != dut_dat)
        		VERBOSE0 ("bs check error at No.%d: shoud be %d, however is %d\n", i, ref_dat, dut_dat);
        }
...
int main (int argc, char* argv[])
{
    ...
    while (1) {
        ...
        if ((fp = fopen("/home/chgu/Desktop/oc-accel/actions/hdl_hevc/sw/enc_top_ori_I8F0S0.yuv","rb"))==NULL) {
